  ...
  <div class="p-4 max-w-md mx-auto">
    ...
    <form @submit.prevent="updateEntry">
      ...
      <div class="mb-6">
        ...
        <div class="relative">
          <input type="range" v-model="entry.intensity" min="1" max="10"
            class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" :style="{
      ...
      <div class="mb-6">
        ...
        <textarea id="entry-text" v-model="entry.text"
          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" rows="4"
          placeholder="Describe your feelings..." required></textarea>
      </div>
...
import { getMoodLog, updateMoodLog } from '@/services/moodService'
...
async function loadEntry() {
  const data = await getMoodLog(route.params.id)
  Object.assign(entry.value, data)
}
...
async function updateEntry() {
  await updateMoodLog(entry.value.log_id || route.params.id, {
    mood: entry.value.mood,
    energy_level: entry.value.energy_level,
    sleep_quality: entry.value.sleep_quality,
    stress_level: entry.value.stress_level,
    notes: entry.value.notes,
    log_date: entry.value.log_date
  })
