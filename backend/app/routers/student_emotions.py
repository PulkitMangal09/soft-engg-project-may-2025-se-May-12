...
def list_emotional_entries(token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("emotional_entries").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
...
def get_emotional_entry(entry_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("emotional_entries").select("*").eq("entry_id", entry_id).eq("user_id", user_id).single().execute()
    data = getattr(resp, 'data', None)
    if not data:
...
def update_emotional_entry(entry_id: str, entry: EmotionalEntry, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("emotional_entries").update(entry.dict(exclude_unset=True)).eq("entry_id", entry_id).eq("user_id", user_id).execute()
    data = getattr(resp, 'data', None)
    if not data:
...
def delete_emotional_entry(entry_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    supabase.table("emotional_entries").delete().eq("entry_id", entry_id).eq("user_id", user_id).execute()
    return {"deleted": True}
...
def list_mood_logs(token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("mood_logs").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
    return getattr(resp, 'data', [])

@router.get("/mood-logs/{log_id}", response_model=MoodLog)
def get_mood_log(log_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("mood_logs").select("*").eq("log_id", log_id).eq("user_id", user_id).single().execute()
    data = getattr(resp, 'data', None)
    if not data:
...
def update_mood_log(log_id: str, log: MoodLog, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("mood_logs").update(log.dict(exclude_unset=True)).eq("log_id", log_id).eq("user_id", user_id).execute()
    data = getattr(resp, 'data', None)
    if not data:
        raise HTTPException(status_code=400, detail="Update failed")
    return data[0]
...
def delete_mood_log(log_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    supabase.table("mood_logs").delete().eq("log_id", log_id).eq("user_id", user_id).execute()
    return {"deleted": True}
...
def get_chat_session(session_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("chat_sessions").select("*").eq("session_id", session_id).eq("user_id", user_id).single().execute()
    data = getattr(resp, 'data', None)
    if not data:
...
def update_chat_session(session_id: str, session: ChatSession, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    resp = supabase.table("chat_sessions").update(session.dict(exclude_unset=True)).eq("session_id", session_id).eq("user_id", user_id).execute()
    data = getattr(resp, 'data', None)
    if not data:
        raise HTTPException(status_code=400, detail="Update failed")
    return data[0]
...
def delete_chat_session(session_id: str, token: str = Depends(oauth2)):
    user_id = get_user_id(token)
    supabase.table("chat_sessions").delete().eq("session_id", session_id).eq("user_id", user_id).execute()
    return {"deleted": True}
...
def get_chat_message(message_id: str, token: str = Depends(oauth2)):
    ...
    msg = supabase.table("chat_messages").select("*").eq("message_id", message_id).single().execute().data
    if not msg:
...
def delete_chat_message(message_id: str, token: str = Depends(oauth2)):
    ...
    if not session or session.get("user_id") != user_id:
        raise HTTPException(status_code=403, detail="Forbidden: Not your session")
    supabase.table("chat_messages").delete().eq("message_id", message_id).execute()
    return {"deleted": True}
...
def list_emergency_contacts(token: str = Depends(oauth2)):
    resp = supabase.table("emergency_contacts").select("*").order("created_at", desc=True).execute()
    return getattr(resp, 'data', [])
...
def get_emergency_contact(contact_id: str, token: str = Depends(oauth2)):
    resp = supabase.table("emergency_contacts").select("*").eq("contact_id", contact_id).single().execute()
    data = getattr(resp, 'data', None)
    if not data:
...
def update_emergency_contact(contact_id: str, contact: EmergencyContact, token: str = Depends(oauth2)):
    resp = supabase.table("emergency_contacts").update(contact.dict(exclude_unset=True)).eq("contact_id", contact_id).execute()
    data = getattr(resp, 'data', None)
    if not data:
        raise HTTPException(status_code=400, detail="Update failed")
    return data[0]
